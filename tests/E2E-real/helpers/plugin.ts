...
export function tabUrl( base: string, tab: string ): string {
	return `${ base.replace( /\/$/, '' ) }/${ PLUGIN_PAGE }&tab=${ tab }`;
}

export async function goTab( page: Page, base: string, tab: string ): Promise< void > {
	await page.goto( tabUrl( base, tab ) );
...
export async function saveProvider( page: Page ): Promise< void > {
	...
	await expect( page ).toHaveURL( /tab=cloud-provider/ );
...
/** Enable offloading from the completion modal; the page reloads into OFFLOADING_ACTIVE. */
export async function enableOffloadingFromModal( page: Page ): Promise< void > {
	...
	await page.waitForURL( /tab=sync/, { timeout: 60_000 } );
	await expect( page.locator( '#disconnect-from-cloud-btn' ) ).toBeVisible( { timeout: 60_000 } );
}
...
/**
 * Disconnect from the cloud: scan, download everything, offloading disabled,
 * page reloads. When every file already exists locally there is nothing to
 * download: the plugin shows the success view and reloads on its own within
 * a couple of seconds, so the view is read the moment it appears.
 */
...
/** Start a disconnect and cancel the download at its first progress update. */
...
export async function removeProvider( page: Page ): Promise< void > {
	...
	await page.waitForURL( /tab=cloud-provider/, { timeout: 60_000 } );
	await expect( page.locator( '#cloud_provider' ) ).toBeVisible();
}

/** Update the access key through the modal: test, then save. */
export async function updateKey( page: Page, key: string, expectSuccess: boolean ): Promise< void > {
	await page.locator( '.update-credentials-trigger' ).click();
	await expect( page.locator( '#update-credentials-modal' ) ).toBeVisible();
	await page.locator( '#modal_account_key' ).fill( key );
	await page.locator( '#modal-test-connection' ).click();
	const result = page.locator( '#modal-connection-result' );
	await expect( result ).toContainText( /success|failed|error|invalid|denied/i, { timeout: 60_000 } );
	if ( expectSuccess ) {
		await expect( result ).toContainText( /success/i );
		await expect( page.locator( '#modal-save-credentials' ) ).toBeEnabled();
		await page.locator( '#modal-save-credentials' ).click();
		await page.waitForURL( /tab=cloud-provider/, { timeout: 60_000 } );
	} else {
		await expect( result ).not.toContainText( /success/i );
		await expect( page.locator( '#modal-save-credentials' ) ).toBeDisabled();
		await page.locator( '#update-credentials-modal .modal-close' ).first().click();
	}
}
