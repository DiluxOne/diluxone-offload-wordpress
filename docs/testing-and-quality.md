...
The settings have tests for what they do, not only for being saved: `ForwardSyncTest` (a file above Maximum File Size never enters the initial sync), `TimeoutTest` (a live upload, a download and a sync batch give up at the Transfer Timeout, and the health records `timeout`), `ForceHttpsTest` (the four URL filters, on the host the provider's URL names) and `LoggingTest` (an upload leaves no informational line with the toggle off, one with it on, and never the key).

`TimeoutTest` waits the setting's minimum, 30 seconds, three times, so it is in the `slow` group. CI runs it; locally, `make test-integration PHPUNIT_ARGS="--exclude-group slow"` leaves it out.
